...
├── VIDEO\_SCRIPT.md                  # Video demonstration script
...
cp target/smart-campus-api.war $GLASSFISH\_HOME/glassfish/domains/domain1/autodeploy/
...
\#### Q5: We explicitly use the @Consumes(MediaType.APPLICATION\_JSON) annotation on the POST method. Explain the technical consequences if a client attempts to send data in a different format, such as text/plain or application/xml.
...
When a resource method is annotated with `@Consumes(MediaType.APPLICATION\_JSON)`, JAX-RS will only invoke that method if the request's `Content-Type` header matches the specified media type.
...
See \[VIDEO\_SCRIPT.md](VIDEO\_SCRIPT.md) for the complete 10-minute demonstration script with timestamps.
...
&#x20;  - `$GLASSFISH\_HOME/glassfish/domains/domain1/logs/server.log`
